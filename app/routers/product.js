...
router.put("/products/:id", authorizePermission(Permission.EDIT_PRODUCT), async (req, res) => {
  const { name, category_id, price, in_stock, description } = req.body;
...
router.delete("/products/:id", authorizePermission(Permission.DELETE_PRODUCT), async (req, res) => {
  const productId = req.params.id;
